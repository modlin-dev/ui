...
export function Item(props: ItemProps) {
	if (props.asChild) {
		...
		const itemProps = {
			className: cn("flex gap-2 p-4 hover:bg-secondary [&>svg]:size-4", props.className, children.props.className)
		}
		return cloneElement(children, mergeProps(children.props, itemProps))
	}
	return <li className={cn("flex gap-2 p-4 hover:bg-secondary [&>svg]:size-4", props.className)} />
}
